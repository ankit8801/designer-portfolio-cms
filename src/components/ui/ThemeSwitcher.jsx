...
export const ThemeSwitcher = () => {
  ...
  return (
    <fieldset className="relative flex p-0 m-0 border border-border-primary/10 bg-section-surface rounded-[999999px]">
      <legend className="sr-only">Select a display theme:</legend>
      
      {/* Sliding Thumb */}
      <div
        className="absolute top-0 left-0 h-full w-8 bg-page-surface border border-border-primary/10 rounded-full z-0 pointer-events-none will-change-transform"
        style={{
          transform: `translateX(${theme === "system" ? 0 : theme === "light" ? 32 : 64}px)`,
      ...
      <div className="relative z-10">
        <input
          ...
          className="appearance-none absolute m-0 p-0 w-0 h-0 overflow-hidden"
        />
        <label
          htmlFor="theme-switch-system"
          className={`flex items-center justify-center h-8 w-8 cursor-pointer rounded-full transition-opacity duration-300 ${
            theme === "system" ? "opacity-100" : "opacity-40 hover:opacity-70"
          }`}
        >
      ...
      <div className="relative z-10">
        ...
        <label
          ...
          className={`flex items-center justify-center h-8 w-8 cursor-pointer rounded-full transition-opacity duration-300 ${
            theme === "light" ? "opacity-100" : "opacity-40 hover:opacity-70"
          }`}
        >
      ...
      <div className="relative z-10">
        ...
        <label
          ...
          className={`flex items-center justify-center h-8 w-8 cursor-pointer rounded-full transition-opacity duration-300 ${
            theme === "dark" ? "opacity-100" : "opacity-40 hover:opacity-70"
          }`}
        >
